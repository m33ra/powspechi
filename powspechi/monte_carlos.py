...
def fct_sin(x, fct, args=[]):
    y = fct(x, *args)*np.sin(x)
    return y
...
def from_fct(fct, args, size, xmin=0., xmax=1.):

    samples = np.array([])
    ymax = np.max(fct(np.linspace(xmin, xmax, 1000), *args))
    ...
    while ii == 0:
        ...
        samples = np.append(samples, ux[uy <= fct(ux, *args)])
